...
@Autonomous
//@Disabled
public class DucksAutonomousRight extends OpMode {
    ...
    public void loop() {
        ...
        if (state == 0) {
            ...
            armPosition = ARM_SCORE_SAMPLE_IN_LOW - 0.7;
            ducksSleepMilliSec(3000);
            MoveForwardDistance(-150, 0.5);
            ducksSleepMilliSec(500);
            state = 6;
        } else if (state == 6) {
            intake.setPower(1.0);
            MoveForwardDistance(-50, 0.5);
            //ducksSleepMilliSec(1200);
            intake.setPower(0);
